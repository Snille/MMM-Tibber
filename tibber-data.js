class TibberData {
  ...
  priceData(showFromTime, showToTime, colorForPriceLevel) {
    ...
    priceData.minPrice = priceData.reduce(
      (min, p) => (p.y < min ? p.y : min),
      priceData[0].y
    );
    priceData.maxPrice = priceData.reduce(
            ...
            data: p.data
              .map((c) => {
                ...
                return {
                  ...
                  y: c.consumption,
                  id:
                    c.consumption === this.maxConsumption()
                      ? "maxCon"
                      : c.consumption === this.minConsumption()
                      ? "minCon"
                      : "",
                };
              })
  ...
  minConsumption() {
    const totalConsumption = this.tibberData.totalConsumption;
    return totalConsumption.reduce(
      (min, c) => (c.consumption < min ? c.consumption : min),
      totalConsumption[0] ? totalConsumption[0].consumption : null
    );
  }

  maxConsumption() {
    const totalConsumption = this.tibberData.totalConsumption;
    return this.tibberData.totalConsumption.reduce(
